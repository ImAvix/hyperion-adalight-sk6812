...
void showAnalogRGB(const CRGBW& led) {
...
void showColor(const CRGBW& led) {
...
void switchOff() {
#if MAX_LEDS > 1 || ANALOG_OUTPUT_ENABLED == false
  memset(leds, 0, MAX_LEDS * sizeof(struct CRGBW));
  FastLED.show();
#endif
...
#if MAX_LEDS > 1 || ANALOG_OUTPUT_ENABLED == false
  FastLED.addLeds<LED_TYPE, LED_PINS, RGB>(ledsRGB, getRGBWsize(MAX_LEDS));
#endif
...
#else
      showAnalogRGB(CRGBW(sum_r / MAX_LEDS, sum_g / MAX_LEDS, sum_b / MAX_LEDS));
#endif
#endif
    }
  }
